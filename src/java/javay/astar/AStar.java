...
//      System.out.println("find@AStar curNode=" + curNode);
      for (int i = 0; i < (blFlag ? 8 : 4); i++) {
        switch (i) {
          case 0:// 右
            if ( (curNode.getY() + 1) < this.height) {
              check(this.searchArea[curNode.getY() + 1][curNode.getX()],
                  endNode, curNode, AStarConstants.COST_ORTHOGONAL); // 10
            }
            break;
          case 1:// 下
            if ( (curNode.getX() + 1) < this.width) {
              check(this.searchArea[curNode.getY()][curNode.getX() + 1],
                  endNode, curNode, AStarConstants.COST_ORTHOGONAL); // 10
            }
            break;
          case 2:// 左
          ...
          case 3:// 上
            ...
            break;
          case 4:// 右上
            if ( (curNode.getY() + 1) < this.height && (curNode.getX() - 1) > 0) {
              check(this.searchArea[curNode.getY() + 1][curNode.getX() - 1],
                  endNode, curNode, AStarConstants.COST_DIAGONAL); // 14
            }
            break;
          case 5:// 右下
            if ( (curNode.getY() + 1) < this.height && (curNode.getX() + 1) < this.width) {
              check(this.searchArea[curNode.getY() + 1][curNode.getX() + 1],
                  endNode, curNode, AStarConstants.COST_DIAGONAL); // 14
            }
            break;
          case 6:// 左上
            if ( (curNode.getY() - 1) > 0 && (curNode.getX() - 1) > 0) {
              check(this.searchArea[curNode.getY() - 1][curNode.getX() - 1],
                  endNode, curNode, AStarConstants.COST_DIAGONAL); // 14
            }
            break;
          case 7:// 左下
            if ( (curNode.getY() - 1) > 0 && (curNode.getX() + 1) < this.width) {
              check(this.searchArea[curNode.getY() - 1][curNode.getX() + 1],
                  endNode, curNode, AStarConstants.COST_DIAGONAL); // 14
            }
            break;
